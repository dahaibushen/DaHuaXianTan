...
//  OneStoreFramework
//
//  Created by huangjiming on 9/25/14.
    ...
    for (id nestedKey in [dictionary.allKeys sortedArrayUsingDescriptors:@[sortDescriptor]]) {
        ...
        if (nestedValue) {
            if ([nestedValue isKindOfClass:[NSDictionary class]]) {
                NSString *jsonStr = [OTSJSONUtil stringFromJSONObject:nestedValue options:0];
                ...
                NSString *jsonStr = [OTSJSONUtil stringFromJSONObject:nestedArray options:0];
                [mQueryPairs addObject:[[OTSQueryPair alloc] initWithKey:nestedKey value:jsonStr]];
            } else {
